...
Nhiệm vụ của bạn là tạo ra đúng {sub_count} câu hỏi trắc nghiệm khách quan có chất lượng cao về chủ đề '{topic_name}' sử dụng công nghệ '{tech_stack}'.

Quy tắc sinh câu hỏi (Quizz Session Skill):
{skill_content}

RÀNG BUỘC CỦA ĐỢT SINH NÀY:
- Số câu hỏi cần tạo: {sub_count} câu.
...
Bạn phải trả về duy nhất một mảng JSON chứa đúng {sub_count} câu hỏi có cấu trúc như sau:
[
  {{
    "STT": {current_stt},
...
"""
        ...
        user_prompt = f"Hãy tạo đúng {sub_count} câu hỏi trắc nghiệm định dạng JSON."
        ...
        for attempt in range(3):
            try:
                ...
                print(f"  [Session Quiz Agent Warning] LLM generation attempt {attempt+1} failed: {e}")
