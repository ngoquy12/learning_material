...
def project_reviewer_agent(entry_tests: List[Dict[str, Any]], srs_doc: Dict[str, Any], mini_project: Dict[str, Any], tech_stack: str) -> Dict[str, Any]:
    ...
    for r_hdr in srs_required:
        if not re.search(r_hdr, srs_content, re.IGNORECASE):
            ...
            return {"status": "REJECTED", "feedback": f"Tài liệu đặc tả SRS thiếu tiêu đề bắt buộc hoặc không đúng định dạng H3 bôi đậm: '{r_hdr_clean}'."}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "predictions" in resp_json and len(resp_json["predictions"]) > 0:
                    ...
                    raise ValueError(f"Imagen API response did not contain images predictions: {resp_json}")
            else:
                raise ValueError(f"Imagen API returned status {response.status_code}: {response.text}")
        except Exception as e:
            raise RuntimeError(f"Failed to generate diagram using Imagen 3: {e}")
    elif not image_path.exists():
        raise ValueError("Cannot generate business diagram because GEMINI_API_KEY or GOOGLE_API_KEY is not configured.")
