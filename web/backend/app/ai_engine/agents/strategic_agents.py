...
def knowledge_base_agent(program_structure: Dict[str, Any], tech_stack: str = "python/core") -> Dict[str, Any]:
    ...
    # Raise error if LLM fails or is not configured
    if not concepts or not code_samples:
        raise RuntimeError(
            "ERROR: Knowledge Base Agent failed to generate SSOT concepts/code samples via LLM. "
            "Please ensure the LLM service is active and credentials are correct."
        )
