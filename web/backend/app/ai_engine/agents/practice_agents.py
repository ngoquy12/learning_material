...
def generate_and_link_diagram(content: str, practice_dir, filename_no_ext: str) -> str:
    ...
    if api_key and not image_path.exists():
        ...
        try:
            ...
            if base_url:
                # Use OpenAI-compatible Image Generation API
                ...
                if response.status_code == 200:
                    ...
                    if img_data and "b64_json" in img_data[0]:
                        ...
                        if img_resp.status_code == 200:
                            ...
                            raise ValueError(f"Failed to download image from url {img_url}: status {img_resp.status_code}")
                    else:
                        raise ValueError(f"Response did not contain images: {resp_json}")
                else:
                ...
                if response.status_code == 200:
                    ...
                    if "predictions" in resp_json and len(resp_json["predictions"]) > 0:
                        ...
                        raise ValueError(f"Response did not contain predictions: {resp_json}")
                else:
                    raise ValueError(f"API returned status {response.status_code}: {response.text}")
        except Exception as e:
            raise RuntimeError(f"Failed to dynamically generate diagram: {e}")
    elif not image_path.exists():
        raise ValueError("Cannot generate diagram because GEMINI_API_KEY or GOOGLE_API_KEY is not configured.")
