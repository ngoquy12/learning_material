...
async def create_lesson(lesson_in: LessonCreate, db: AsyncSession = Depends(get_db)):
    # get max order
    query = select(Lesson.order_index).where(Lesson.session_id == lesson_in.session_id).order_by(Lesson.order_index.desc())
...
from app.ai_engine.core.graph import compile_learning_content_workflow
from app.ai_engine.core.state import AgentState

async def generate_lesson_task(lesson_id: int, session_id: int, pm_input: str, title: str, technology_stack: str = "python/fastapi", course_dir_name: str = "Database_Course"):
    print(f"\n[AI TRIGGER] Bắt đầu sinh học liệu cho Lesson {lesson_id} - {title}")
    
    from app.db.session import AsyncSessionLocal as SessionLocal
    from app.models.lesson import Lesson as DbLesson
    ...
    async with SessionLocal() as db:
        try:
            lesson_obj = await db.get(DbLesson, lesson_id)
            if lesson_obj:
    ...
    is_practical = ("thực hành" in lesson_title_lower or "thuc hanh" in lesson_title_lower or "practice" in lesson_title_lower or "project" in lesson_title_lower or "dự án" in lesson_title_lower or "thực hành" in details_lower or "thuc hanh" in details_lower)
    ...
    from app.ai_engine.core.persistence import load_checkpoint
    from app.db.session import AsyncSessionLocal as SessionLocal
    from app.models.artifact import Artifact
        ...
        while not task.done():
            ...
            try:
                ...
                artifacts_to_save = {
                    ...
                    "outline": (state.get("mindmap_markdown") or state.get("slide_markdown"), state.get("artifacts_status", {}).get("mindmap")),
                    "walkthrough": (state.get("video_script_markdown") or state.get("html_content"), state.get("artifacts_status", {}).get("video_script"))
                }
    ...
    try:
        ...
        async with SessionLocal() as db:
            ...
            try:
                ...
                if all_lesson_ids:
                    pending_stmt = select(Artifact).where(Artifact.lesson_id.in_(all_lesson_ids), Artifact.status == "Pending")
                    pending_res = await db.execute(pending_stmt)
                    pending_artifacts = pending_res.scalars().all()
                    
                    if not pending_artifacts:
                        print(f"[AI Session Trigger] All lessons completed, auto-triggering session artifacts compilation for session {session_id}...")
