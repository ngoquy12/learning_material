...
LANG_CODES = dict(
    ...
    # espeak-ng except Vietnamese, which uses vig2p below.
    d='de',
    e='es',
    f='fr-fr',
    h='hi',
...
VI_TEXT_TOKEN_RE = re.compile(r"[A-Za-zÀ-ỹĐđ]+(?:[-'][A-Za-zÀ-ỹĐđ]+)*|\s+|.", re.UNICODE)
VI_WORD_RE = re.compile(r"^[A-Za-zÀ-ỹĐđ]+(?:[-'][A-Za-zÀ-ỹĐđ]+)*$", re.UNICODE)
VI_MARK_RE = re.compile(r"[À-ỹĐđ]")
VI_NON_VI_S_CLUSTERS = ('sc', 'sh', 'sk', 'sl', 'sm', 'sn', 'sp', 'st', 'sw')


def _fix_vi_phonemes(phonemes: str, source_text: str | None = None) -> str:
    ...
    if source_text:
        ...
        elif source_lower.startswith('s') and not source_lower.startswith(VI_NON_VI_S_CLUSTERS):
...
def _phonemize_vi_text(text: str, g2p) -> str:
    try:
        ...
        pass
    raw_phonemes = g2p.many([text])[0]
    return _fix_vi_phonemes(raw_phonemes, source_text=text)
        ...
        for graphemes_index, graphemes in enumerate(text):
            ...
            # Non-English processing with chunking
                ...
                for chunk in chunks:
                    ...
                    if self.lang_code == 'v':
                        ps = _phonemize_vi_text(chunk, self.g2p)
                    else:
                        ps, _ = self.g2p(chunk)
                    if not ps:
                        continue
