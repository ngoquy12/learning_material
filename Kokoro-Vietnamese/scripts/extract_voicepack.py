#!/usr/bin/env python3
"""
Kokoro Vietnamese: Extract Voicepack
====================================
...
Usage:
    ...
    python scripts/extract_voicepack.py \
        --model StyleTTS2/logs/kokoro-vi-30h/first_stage.pth \
        --audio-dir dataset_vi_30h/audio \
        --output voices/vi_voice.pt

    # Two checkpoints (recommended after Stage 2 training):
    #   style_encoder from Stage 1, predictor_encoder from Stage 2
    python scripts/extract_voicepack.py \
        --model StyleTTS2/logs/kokoro-vi-30h/epoch_2nd_00008.pth \
        --style-encoder-model StyleTTS2/logs/kokoro-vi-30h/first_stage.pth \
        --audio-dir dataset_vi_30h/audio \
        --output voices/vi_voice.pt

    # Multi-speaker voicepack for one speaker label/id
    python scripts/extract_voicepack.py \
        --model StyleTTS2/logs/kokoro-vi-30h-ms/epoch_2nd_00008.pth \
        --style-encoder-model StyleTTS2/logs/kokoro-vi-30h-ms/first_stage.pth \
        --audio-dir dataset_vi_30h/audio \
        --metadata dataset_vi_30h/metadata.csv \
        --speaker speaker_a \
        --output voices/speaker_a.pt
...
def _read_voicepack_metadata(metadata_path: Path) -> list[dict[str, str]]:
    rows = []
    with metadata_path.open('r', encoding='utf-8') as f:
        for line_no, raw_line in enumerate(f, 1):
            ...
            if len(parts) < 2:
                raise ValueError(f'{metadata_path}:{line_no}: expected filename|text|speaker')
            filename = parts[0]
            text = parts[1]
            if filename.lower() in {'filename', 'path', 'audio'} and text.lower() == 'text':
                continue
            speaker_id = parts[2] if len(parts) >= 3 and parts[2] else '0'
            speaker_label = parts[3] if len(parts) >= 4 and parts[3] else speaker_id
            rows.append(
                {
                    'filename': filename,
...
def _metadata_audio_path(audio_dir: Path, filename: str) -> Path:
    path = Path(filename)
    if path.is_absolute():
        return path
    direct = audio_dir / path
    if direct.exists():
        return direct
    return audio_dir / path.name


def select_voicepack_audio_files(
    audio_dir: str | Path,
    metadata_path: str | Path | None = None,
    speaker: str | None = None,
) -> list[Path]:
    audio_path = Path(audio_dir)
    if metadata_path is None or speaker is None or speaker == '':
        return sorted(audio_path.glob("*.wav"))

    speaker = str(speaker)
    rows = _read_voicepack_metadata(Path(metadata_path))
    selected = []
    for row in rows:
        if speaker not in {row['speaker_id'], row['speaker_label']}:
            continue
        wav_path = _metadata_audio_path(audio_path, row['filename'])
        if wav_path.exists():
...
def extract_voicepack(
    ...
    # ── Collect audio files ──────────────────────────────────────────────
    wav_files = select_voicepack_audio_files(audio_dir, metadata_path, speaker)
    if not wav_files:
        speaker_msg = f" for speaker {speaker!r}" if speaker else ""
        print(f"ERROR: No WAV files found in {audio_dir}{speaker_msg}")
        sys.exit(1)

    speaker_msg = f" for speaker {speaker!r}" if speaker else ""
    print(f"Found {len(wav_files):,} WAV files in {audio_dir}{speaker_msg}")
...
Examples:
  ...
  # Multi-speaker voicepack for a selected speaker id/label
  python scripts/extract_voicepack.py \\
      --model StyleTTS2/logs/kokoro-vi-30h-ms/epoch_2nd_00008.pth \\
      --style-encoder-model StyleTTS2/logs/kokoro-vi-30h-ms/first_stage.pth \\
      --audio-dir dataset_vi_30h/audio \\
...
""",
    ...
    parser.add_argument(
        "--metadata",
        default=None,
        help="Optional dataset metadata.csv with filename|text|speaker rows. Required for --speaker filtering.",
    )
    parser.add_argument(
        "--speaker",
        default=None,
        help="Optional speaker id or speaker label to extract a speaker-specific voicepack.",
    )
    parser.add_argument(
        "--output",
        required=True,
