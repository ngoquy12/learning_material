...
def homework_reviewer_agent(
    ...
    for idx, ex in enumerate(exercises):
        ...
        if re.search(r"\bAI\b", combined_text_for_ai_check):
            return {"status": "REJECTED", "feedback": f"Bài tập {idx+1} '{title}' chứa từ viết tắt 'AI'. Hãy tránh nhắc đến AI hoặc trợ lý ảo ngoài phần tiêu đề Rubric."}
        ...
        if idx+1 in [1, 2]:
            ...
            if expected_submit not in content:
                return {"status": "REJECTED", "feedback": f"Bài tập {idx+1} '{title}' thiếu quy định nộp bài chuẩn GitHub format kết thúc bằng '{expected_submit}'."}
        elif idx+1 == 3:
        ...
        if idx_problem == -1 or idx_req == -1:
            return {"status": "REJECTED", "feedback": f"Bài tập {idx+1} '{title}' không định nghĩa đúng cấu trúc phần Vấn đề hoặc Quy tắc nghiệp vụ / Mã nguồn hiện tại."}
        ...
        if prompt_match:
            ...
            if not any(kw in prompt_text for kw in english_keywords):
                return {"status": "REJECTED", "feedback": f"Bài tập {idx+1} '{title}' có Prompt tạo ảnh thiếu các mô tả phong cách thiết kế phẳng, tối giản (flat vector style, minimal flow diagram)."}
            if not ("vietnamese" in prompt_text or "tiếng việt" in prompt_text):
                return {"status": "REJECTED", "feedback": f"Bài tập {idx+1} '{title}' có Prompt tạo ảnh chưa chỉ định rõ ngôn ngữ tiếng Việt cho phần chú thích mô tả."}
            if not any(ek in prompt_text for ek in ["english", "technical terms", "keep"]):
                return {"status": "REJECTED", "feedback": f"Bài tập {idx+1} '{title}' có Prompt tạo ảnh thiếu yêu cầu giữ nguyên thuật ngữ kỹ thuật chuyên ngành bằng tiếng Anh."}
        ...
        if not rubric.startswith("### **Tiêu chí chấm điểm (AI)**"):
        ...
        # Verify level-specific rubric categories
        if idx+1 in [1, 2]:
            rubric_categories = [
                r"#### \*\*1\.\s+Phân tích\s+&\s+Phát hiện",
                r"#### \*\*2\.\s+Hiện thực\s+sửa\s+lỗi",
                r"#### \*\*3\.\s+Kiểm\s+chuẩn\s+dữ\s+liệu",
                r"#### \*\*4\.\s+Lý\s+thuyết\s+mở\s+rộng",
                r"#### \*\*5\.\s+Chất lượng mã nguồn",
                r"#### \*\*Điểm cộng"
            ]
        elif idx+1 == 3:
            rubric_categories = [
                r"#### \*\*1\.\s+Báo cáo phân tích",
                r"#### \*\*2\.\s+Lập trình logic",
                r"#### \*\*3\.\s+Kiểm chuẩn dữ liệu",
                r"#### \*\*4\.\s+Xử lý ngoại lệ",
                r"#### \*\*5\.\s+Chất lượng mã nguồn",
                r"#### \*\*Điểm cộng"
            ]
        elif idx+1 == 4:
            rubric_categories = [
                r"#### \*\*1\.\s+Báo cáo Đề xuất",
                r"#### \*\*2\.\s+Giải trình Lựa chọn",
                r"#### \*\*3\.\s+Triển khai mã nguồn",
                r"#### \*\*4\.\s+Kiểm chuẩn dữ liệu",
                r"#### \*\*5\.\s+Chất lượng mã nguồn",
                r"#### \*\*Điểm cộng"
            ]
        elif idx+1 == 5:
            rubric_categories = [
                r"#### \*\*1\.\s+Thiết kế kiến trúc",
                r"#### \*\*2\.\s+Hiện thực hóa logic",
                r"#### \*\*3\.\s+Kiểm chuẩn dữ liệu",
                r"#### \*\*4\.\s+Chất lượng mã nguồn",
                r"#### \*\*Điểm cộng"
            ]
        elif idx+1 == 6:
            rubric_categories = [
                r"#### \*\*1\.\s+Khởi tạo Dự án",
                r"#### \*\*2\.\s+Hiện thực hóa các API",
                r"#### \*\*3\.\s+Kiểm chuẩn logic",
                r"#### \*\*4\.\s+Chất lượng mã nguồn",
                r"#### \*\*5\.\s+Quy chuẩn nộp bài"
            ]
            
        for cat in rubric_categories:
            if not re.search(cat, rubric, re.IGNORECASE):
                return {"status": "REJECTED", "feedback": f"Tiêu chí chấm điểm Bài tập {idx+1} '{title}' thiếu nhóm tiêu chí bắt buộc: '{cat.replace('\\', '')}'."}
        ...
        if idx+1 in [1, 2]:
            if "```python" not in content and "```" not in content:
                return {"status": "REJECTED", "feedback": f"Bài tập {idx+1} (Cơ bản) '{title}' thiếu mã nguồn hiện tại bị lỗi logic để sinh viên debug."}
        else: # Levels 3, 4, 5, 6
            ...
            for code_block in python_code_blocks:
                ...
                if any(kw in code_text for kw in ["def ", "class ", "import ", "try:", "except ", "return "]):
                    return {"status": "REJECTED", "feedback": f"Bài tập {idx+1} (Mức {level}) '{title}' vi phạm Chính sách Cấm Gợi ý Code (No-Code Hinting Policy). Hãy xóa toàn bộ khung code/cấu trúc hàm và chỉ giữ lại ví dụ dữ liệu."}
            
            for block in generic_code_blocks:
                if any(kw in block for kw in ["def ", "class ", "import ", "return "]):
                    return {"status": "REJECTED", "feedback": f"Bài tập {idx+1} (Mức {level}) '{title}' vi phạm Chính sách Cấm Gợi ý Code. Đã phát hiện cú pháp mã nguồn lập trình trong khối code."}

        # 12. Check that there is at least one edge case/bẫy dữ liệu
        edge_case_keywords = ["bẫy dữ liệu", "ràng buộc", "trùng", "vượt", "không tồn tại", "bẫy lỗi", "ngoại lệ", "edge case", "validate", "kiểm chuẩn"]
        if not any(kw in content.lower() for kw in edge_case_keywords):
            return {"status": "REJECTED", "feedback": f"Bài tập {idx+1} '{title}' thiếu các cài cắm bẫy dữ liệu nghiệp vụ (Edge Cases)."}

        # 13. ENFORCE STRICT KNOWLEDGE RANGE BOUNDARY (NO UNLEARNED TOPICS)
        # - Checks if the generated content references advanced frameworks or libraries that are not in previous_lessons_text
        ...
        for word in advanced_keywords:
            if word in combined_text.lower():
                if word not in previous_lessons_text.lower() and word not in tech_stack.lower():
                    # Handle special edge case of the word "session" in standard context (e.g. Session 02) vs database session
                    if word == "session" and "database session" not in combined_text.lower() and "cookie session" not in combined_text.lower():
                        continue
                    return {"status": "REJECTED", "feedback": f"Bài tập {idx+1} '{title}' vi phạm giới hạn kiến thức. Chứa từ khóa kỹ thuật chưa được học: '{word}'. Bạn chỉ được dùng kiến thức trong phạm vi các bài học trước/hiện tại."}

    return {"status": "APPROVED", "feedback": f"Hệ thống bài tập lý thuyết đạt tiêu chuẩn sư phạm với phân hệ nghiệp vụ doanh nghiệp {chosen_domain.upper()}."}
