...
### Câu 1: Trong kịch bản khởi tạo và quản lý kho lưu trữ ở bài đọc môn python, hãy phân tích hậu quả xảy ra khi lập trình viên thực thi lệnh mà quên sử dụng cờ `--global` và nêu cách kiểm tra lại file cấu hình.
> **Gợi ý trả lời & Định hướng đáp án:** Khi không có cờ `--global`, cấu hình chỉ áp dụng cho local repository hiện tại. Cần kiểm tra lại bằng lệnh `python config --list` hoặc xem file `.git/config` để xác nhận phạm vi.
