  ...
  <head>
    ...
    <script>
      function renderAllMermaidDiagrams() {{
        document.querySelectorAll('.mermaid, .language-mermaid, pre.mermaid, code.language-mermaid, div[class*="mermaid"]').forEach(el => {{
          if (el.dataset.mermaidConverted) return;
          const text = (el.innerText || el.textContent || '').trim();
          if (text.startsWith('sequenceDiagram') || text.startsWith('flowchart') || text.startsWith('graph ') || text.startsWith('gantt') || text.startsWith('classDiagram')) {{
            el.dataset.mermaidConverted = "true";
            if (!el.classList.contains('mermaid')) {{
              el.classList.add('mermaid');
            }}
            el.style.cssText = 'display:flex;justify-content:center;margin:20px 0;background:#0f172a;padding:20px;border-radius:12px;border:1px solid #334155;color:#f8fafc;width:100%;overflow-x:auto;';
          }}
        }});
        if (window.mermaid) {{
          try {{
            mermaid.initialize({{ startOnLoad: true, theme: 'dark', securityLevel: 'loose', suppressErrorRendering: true }});
