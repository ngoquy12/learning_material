...
def compile_session_slides(session_dir: Path, session_title: str):
    ...
    for idx, l_dir in enumerate(lesson_dirs, 1):
        ...
        scenes = []
        if slide_html_p.exists():
            try:
                with open(slide_html_p, "r", encoding="utf-8") as f:
                    file_text = f.read()
                ...
                for s_div in slide_divs:
                    ...
                    # Strip emojis
                    inner_html = re.sub(r'[𐀀-􏿿☀-⛿✀-➿]', '', inner_html)
                ...
                print(f"  [Warning] Error parsing {slide_html_p}: {e}")

        elif slide_md_p.exists():
            try:
                md_text = slide_md_p.read_text(encoding="utf-8")
                # Parse markdown scenes split by --- or # Slide
                ...
                print(f"  [Warning] Error parsing {slide_md_p}: {e}")
