...
<html lang="vi">
  <head>
    ...
    <!-- Mermaid JS Flowchart & Sequence Diagram Engine -->
    <script src="https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js"></script>
    <script>
      function renderAllMermaidDiagrams() {{
        document.querySelectorAll('pre code, .code-body code, div.code-container, pre').forEach(el => {{
          const text = (el.innerText || el.textContent || '').trim();
          if (text.startsWith('sequenceDiagram') || text.startsWith('flowchart') || text.startsWith('graph TD') || text.startsWith('graph LR') || text.startsWith('gantt') || text.startsWith('classDiagram')) {{
            const targetParent = el.closest('.code-container, pre') || el;
            if (targetParent && !targetParent.dataset.mermaidConverted) {{
              targetParent.dataset.mermaidConverted = "true";
              const mermaidDiv = document.createElement('div');
              mermaidDiv.className = 'mermaid';
              mermaidDiv.style.cssText = 'display:flex;justify-content:center;margin:20px 0;background:#0f172a;padding:20px;border-radius:12px;border:1px solid #334155;color:#f8fafc;width:100%;overflow-x:auto;';
              mermaidDiv.textContent = text;
              targetParent.parentNode.replaceChild(mermaidDiv, targetParent);
            }}
          }}
        }});
        if (window.mermaid) {{
          try {{
            ...
            console.log('Mermaid render info:', e);
          }}
        }}
      }}
