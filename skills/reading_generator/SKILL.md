...
|  4  | **2D Flat Vector Image Integration (No HTML/CSS SVG Manual Code)** — 2D Flat Vector Technical Scene Image (via `image_prompt_standard` skill) rendered compact (`max-h-[300px] max-w-2xl w-auto h-auto mx-auto rounded-xl object-contain`) with NO border, NO background box, NO text clipping, no page scrolling needed, Code Sandboxes per sub-section (2.1, 2.2...), Step Code Visualizer | MANDATORY |
